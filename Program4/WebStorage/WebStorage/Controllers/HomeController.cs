...
namespace WebStorage.Controllers
{
    public class HomeController : Controller
    {
        ...
        private void Query()
        {
            string storageConnection = "DefaultEndpointsProtocol=https;AccountName=webstorageapi;AccountKey=BOcD+JlMzzHjh61u2Sit8XqCqU5T+HrLJdSPxOG+xFXQ7EUESZKXezmKDrMUbZK9BJ0H6jzBx50zmt5+JmeMug==;EndpointSuffix=core.windows.net";             CloudStorageAccount cloudStorageAccount = CloudStorageAccount.Parse(storageConnection);             CloudTableClient tableClient = cloudStorageAccount.CreateCloudTableClient();             CloudTable cloudTable = tableClient.GetTableReference("customers");             TableQuery<Customer> tableQuery = new TableQuery<Customer>();             foreach (Customer customerEntity in cloudTable.ExecuteQuery(tableQuery))             {                 Console.WriteLine("Customer First Name : {0}", customerEntity.FirstName);                 Console.WriteLine("Customer Last Name : {0}", customerEntity.LastName);                 Console.WriteLine("Customer Attributes : {0}", customerEntity.Attributes);                 Console.WriteLine("******************************");             }
        }
    }
}